...
void Controller::receive(std::unique_ptr<Event> event) {
    int realEvent = event->getMessageId();

    switch (realEvent) {
        case (TimeoutInd::MESSAGE_ID) : handleTimePassed(*static_cast<EventT<TimeoutInd> const&>(*event));
        break;
        case (DirectionInd::MESSAGE_ID) : handleDirectionChange(*static_cast<EventT<DirectionInd> const&>(*event));
        break;
        case (FoodInd::MESSAGE_ID) : handleFoodPositionChange(*static_cast<EventT<FoodInd> const&>(*event));
        break;
        case (FoodResp::MESSAGE_ID) : handleNewFood(*static_cast<EventT<FoodResp> const&>(*event));
        break;
